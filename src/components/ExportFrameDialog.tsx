...
import { Download, X, Image as ImageIcon, FileType } from 'lucide-react';
...
export function ExportFrameDialog({ isOpen, onClose, videoRef, currentTime }: ExportFrameDialogProps) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (isOpen) {
      const defaultName = `frame_${formatTimeForFile(currentTime)}`;
      setFileName(defaultName);
    }
  }, [isOpen, currentTime]);

  const formatTimeForFile = (time: number) => {
        ...
        <div className="p-6 space-y-5">
          <div>
            ...
            <input
              ...
              placeholder={t('exportFrame.fileNamePlaceholder')}
            />
          </div>
          ...
          {(format === 'jpg' || format === 'jpeg' || format === 'webp') && (
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-2">
                {t('exportFrame.quality')}: {(quality * 100).toFixed(0)}%
              </label>
              <input
                type="range"
                min="0.1"
          ...
          <div className="p-3 bg-zinc-800/50 rounded-lg">
            <p className="text-xs text-zinc-500">
              {t('exportFrame.preview')}: <span className="text-zinc-300 font-mono">{fileName}.{format}</span>
            </p>
          </div>
        </div>
